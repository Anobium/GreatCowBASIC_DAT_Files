...
FOR CurrentChip = StartChip to ChipCount
  ...
  If ThisChipData <> "" Then
    ...
    'Dont read the ADC value
   ' TempData = Mid(TempData, INSTR(TempData, ",") + 1)

'    Consume theprint "3 "+ChipPackageList ADC value
    ChipPackageList = Mid(TempData, INSTR(TempData, ",") + 1)
    'trim the quotes
    if INSTR(ChipPackageList,chr(34)) = 1 then

      if MID(ChipPackageList,len(ChipPackageList),1) = chr(34) then

          ChipPackageList = mid(ChipPackageList,2,len(ChipPackageList)-2)
  ...
  End If
    ...
    PRINT #1, "'Format last revised: 5/4/2019"
    PRINT #1, ""
