...
Dim As Integer RC, CW, T, DisableRead, CVNC, CheckSFR, CheckSFR2, CheckSFRBit
...
Dim Shared As Double ChipWords, ChipMHz, ChipIntOsc
Dim Shared As String ChipIntOscSpeeds

Dim Shared As String InstDir, IncFileDir, DevFileDir, MPASMInfoFile, OutputDir, VDIFileDir, ChipName
Dim Shared As Integer DefConf, ConfMask, DFN, ChipPins, COC, PinoutCount, PowerPinsAdded
Dim Shared As Integer HEFMemory, SAFMemory, ProgMemRowSize, ProgMemRowSizeMandate, PWMTimerVariant, SMTClockSourceVariant, ConfigBaseLocVariant, IntOSCCONFormatVariant, READAD10BITFORCEVariant, Stacks, IDAddress, IDLength
...
'print "3";DataSource
            ...
            IF Left(DataSource, 7) = "_CONFIG" THEN RC = 2
...
'print "5";DataSource
   ...
   Else
    ...
    Else
      ...
      NoBankRAMStr = trim(Mid(TempData, INSTR(TempData, ",") + 1))
       '= ReplaceStr(TempData,"|",chr(10)+Chr(13))
...
'    End If
  ...
  End If
    ...
    IF Left(ChipName, 2) = "18" THEN
        ...
        RemDupConf:
        T = T + 1
            IF DefConfig(T, 0) = DefConfig(T + 1, 0) THEN
...
'    PRINT #1, "'Generated " + CurrentDate
    ...
    PRINT #1, "'Header Format last revised: 28/03/2019"
    PRINT #1, ""
    ...
    End If
    IF ChipFamily <> 16 Then Print #1, "ConfigWords=" + Str(ConfWords)
    Print #1, "PSP=" + Str(PSP)
    ...
    PRINT #1, "[Registers]"
    FOR PD = 1 to SVC
        If Val("&H" + SysVars(PD, 2)) <= MaxChipAddress Then
...
Sub AddMissingData
  ...
  If ChipFamily = 16 Then
    ...
    FOR CD = 1 to ConfigOptions
      If ConfigOption(CD).Name = "DEBUG" THEN GoTo FoundDebugConfigBit
    Next

    'If it doesn't have any config, don't try adding 1!
    If ConfigOptions = 0 Then GoTo FoundDebugConfigBit
...
Sub CalcIntOscSpeeds
  ...
  If HasSFR("OSCCON") Then
    ...
    If HasSFRBit("SPLLEN") Or HasSFRBit("PLLEN") Then
      Select Case ChipIntOsc
        Case 64:
        if InStr(LCase(chipname), "k20")<>0 or InStr(LCase(chipname), "k22")<>0 or InStr(LCase(chipname), "k80")<>0 then
  ...
  ElseIf HasSFR("OSCCON2") Then
    Select Case ChipIntOsc
      Case 64: ChipIntOscSpeeds = "64, 48, 32, 24, 16, 8, 4, 2, 1, 0.5, 0.25, 0.125"
...
Function GetIntName (IntVect As String) As String
  ...
  Select Case UCASE(IntVect)
    ...
    Case "SPI1RXIE"
        IntName = "SPI1ReceiveInterrupt"
    Case "SPI1TXIE"
        IntName = "SPI1TransmitInterrupt"
    Case "SPI1IE"
        IntName = "SPI1Interrupt"
    Case "I2C1RXIE"
...
Sub ReadMPASMInfo
  ...
  Dim As Integer OptionMask, CurrValue
  Dim As String InLine, LineItem(50)

  Dim As LongInt CurrWordLoc
  Dim As Integer CurrWordMask, CurrSettingMask, CurrSettingValMask, DefWordValue(50)
